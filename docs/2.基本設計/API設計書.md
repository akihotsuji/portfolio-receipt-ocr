...
2. ジョブをキューに投入（ワーカーが非同期に CSV 生成）
3. 生成した CSV を MinIO に保存
...
```
1. POST /api/upload-batches でアップロード → batchId を取得
2. 2秒ごとに GET /api/upload-batches/{batchId} をポーリング
3. summary.queued + summary.processing が 0 になったらポーリング停止
4. COMPLETED のジョブは確認・編集フォームを表示可能にする
5. FAILED のジョブはリトライボタンを表示する
```
...
画面A                        画面B
──────                      ──────────────────────────────────
ファイル選択                    ステータス一覧（ポーリング）
    │                              │
    ▼                              ▼
POST /upload-batches ──→ GET /upload-batches/{batchId} (×N回)
                               │
                          ジョブ COMPLETED
                               │
                               ▼
