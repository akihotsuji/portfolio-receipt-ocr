...
RuntimeException
  └── DomainException                         # ドメイン例外の基底クラス（shared）
        ...
        └── ExportException                    # エクスポート固有の例外（Spring Boot 側）
...
エクスポートの CSV 生成処理は Lambda で実行されるため、Lambda 側では Spring の例外体系を使わず、素の Java 例外でエラーを処理する。Lambda がスローした例外は SQS の再配信メカニズムに委ねられる。
...
public enum UploadBatchErrorCode implements ErrorCode {
    FILE_TOO_LARGE("FILE_TOO_LARGE", "ファイルサイズが上限（2MB）を超えています", ErrorCategory.VALIDATION),
...
| エラーコード | HTTP | 発生条件 |
|---|---|---|
| FILE_TOO_LARGE | 400 | 1ファイルが 2MB を超過 |
...
### 6.4 Infrastructure 層（OCR Worker / MessageListener）
...
### 6.5 Lambda 関数（エクスポート CSV 生成）

エクスポート処理は Lambda 関数で実行されるため、Spring の例外体系は使用しない。

| 方針 | 説明 |
|---|---|
| 処理の失敗 | DB ステータスを FAILED に更新後、`RuntimeException` をスロー → Lambda が異常終了 → SQS が自動再配信 |
| タイムアウト | Lambda のタイムアウト設定（300秒）で制御。タイムアウト時は Lambda ランタイムが強制終了し、SQS メッセージが再配信される |
| コールドスタート失敗 | DB 接続の初期化に失敗した場合は例外をスローし、SQS が再配信する。次回起動時に新しいコネクションで再試行 |
| error_message の記録 | `export_jobs.error_message` にエラー概要を記録する方針は Spring Boot 側と同じ |

```java
// ExportCsvHandler の例
public Void handleRequest(SQSEvent event, Context context) {
    for (SQSEvent.SQSMessage message : event.getRecords()) {
        ExportJobMessage jobMessage = parseMessage(message.getBody());
        try {
            processExport(jobMessage);
        } catch (Exception e) {
            logger.error("エクスポート処理に失敗しました: exportId={}", jobMessage.exportId(), e);
            updateStatusToFailed(jobMessage.exportId(), e.getMessage());
            throw new RuntimeException(e);  // SQS の自動再配信に任せる
        }
    }
    return null;
}
...
|---|---|---|
| FILE_TOO_LARGE | 「{ファイル名} のサイズが上限（2MB）を超えています」 | 該当ファイルをエラー表示 |
...
| 状況 | 表示メッセージ | UI の挙動 |
|---|---|---|
| 元ファイルが 20MB 超 | 「{ファイル名} のサイズが大きすぎます（上限: 20MB）」 | 該当ファイルをエラー表示（圧縮を試行しない） |
| 圧縮後も 2MB 超 | 「{ファイル名} を圧縮しましたがサイズが上限（2MB）を超えています」 | 該当ファイルをエラー表示 |
| 圧縮処理の失敗 | 「{ファイル名} の圧縮に失敗しました」 | 該当ファイルをエラー表示 |

> 画像圧縮は API Gateway HTTP API の 10MB ペイロード制限に対応するために導入。Canvas API（OffscreenCanvas）で長辺 1600px にリサイズし、JPEG 品質 0.8 で再エンコードする。PDF は圧縮対象外。

#### ocrjob（画面B）

| エラーコード | 表示メッセージ | UI の挙動 |
|---|---|---|
| — (一覧取得失敗) | 「バッチ一覧の取得に失敗しました」 | BatchHistorySidebar にエラーメッセージ + 「再試行」リンクを表示 |
| NOT_FOUND | 「バッチが見つかりません」または「ジョブが見つかりません」 | Notification でエラー表示。バッチ一覧は表示を維持 |
...
```mermaid
flowchart TD
    A[SQS イベントで Lambda 起動] --> B[status → PROCESSING に更新]
    B --> C[export_job_receipts から<br>対象レシート取得]
    C -->|データなし| D[EXPORT_EMPTY_SELECTION]
    ...
    F --> J
    H --> J
    J --> K[RuntimeException スロー<br>Lambda 異常終了<br>SQS 自動再配信]
```
...
| 整合性 | 制約値（2MB、10枚、許可形式）は定数として管理し、乖離を防ぐ |
